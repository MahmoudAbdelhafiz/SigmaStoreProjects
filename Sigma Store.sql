select *
from Sigmastore ..customers


--How many customers are male and female ?

select gender , count(*) from Sigmastore ..customers
group by gender;
...
select count(*) from Sigmastore ..customers where dob is null;


--How many customers are there in each pincode and gender combination?

select primary_pincode, gender, count(cust_id) from Sigmastore ..customers
...
from Sigmastore ..products 
where mrp>50000;



select*
from Sigmastore ..delivery_person
...
select*
from Sigmastore ..orders


--For each Pin code, print the count of orders, sum of total amount paid, average amount paid, maximum amount paid,
-- minimum amount paid for the transactions which were paid by 'cash'. 
--Take only 'buy' order types


select delivery_pincode as delivery_pincode , count(order_id), sum(total_amount_paid), avg(total_amount_paid), max(total_amount_paid)
...
--For each delivery_person_id, print the count of orders and 
...
select delivery_person_id as delivery_person_id, count(order_id), sum(total_amount_paid) as tap from Sigmastore ..orders
...
select concat(first_name, last_name) as Name from Sigmastore ..customers
where email like '%gmail%';


--How many orders had #units between 1-3, 4-6 and 7+? Take only 'buy' order types
...
count(order_id) from Sigmastore ..orders
...
select delivery_pincode as delivery_pincode, avg(total_amount_paid) as avge
from Sigmastore ..orders
where order_type='buy'
 group by delivery_pincode
...
select p.state, count(distinct c.cust_id) as Customers, count(distinct dp.person_id) as Delivery_Man 
 from Sigmastore ..pincode p inner join customers c
